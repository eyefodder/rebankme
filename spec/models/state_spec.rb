...
describe State do
  ...
  describe 'chase_state?' do
    let(:in_states) do
      %w(AZ CA CO CT FL GA HI ID IL IN KY LA MA MI
         NJ NV NY OH OK OR TX UT WA WI WV)
    end
    it 'returns false for the rest' do
      State.all.each do |state|
        expected = in_states.include?(state.code) ? true : false
        fail_msg = "Expected chase_state for #{state.code} to be #{expected}"
        expect(state.chase_state?).to eq(expected), fail_msg
      end
    end
  end
  describe 'us_bank_states?' do
    let(:in_states) do
      %w(AZ AR CA CO ID IL IN IA KA KY MN MO MT NE
         NV ND OH OR SD TN UT WA WI WY)
    end
    it 'returns false for the rest' do
      State.all.each do |state|
        expected = in_states.include?(state.code) ? true : false
        fail_msg = "Expected us_bank_state? for #{state.code} to be #{expected}"
        expect(state.us_bank_state?).to eq(expected), fail_msg
      end
    end
  end
  ...
  describe 'seeded content' do
    ...
    it 'has an entry for each state' do
      states.each do |state|
        fail_msg = "Couldnt find a state with code #{state[:code]}" \
                   " and name #{state[:name]}"
        result = State.find_by(name: state[:name], code: state[:code])
